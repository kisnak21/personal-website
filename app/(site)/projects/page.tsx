...
export default async function ProjectsPage() {
  let projectsList: import('@/lib/types').Project[] = []
  try {
    ...
    <>
      ...
      <div className='flex bg-surface-container-low border-b border-outline-variant h-10 items-center px-margin-mobile md:px-margin-desktop'>
        <div className='bg-surface px-4 h-full flex items-center gap-2 border-r border-outline-variant border-t-2 border-t-primary'>
          <span className='material-symbols-outlined text-primary text-[14px]'>data_object</span>
          <span className='font-code-sm text-code-sm text-primary'>projects.json</span>
        </div>
      </div>
      ...
      <div className='max-w-[1200px] mx-auto px-margin-mobile md:px-margin-desktop py-8'>
        <div className='grid grid-cols-1 lg:grid-cols-12 gap-8'>
          ...
          {/* Desktop: side-by-side JSON code panel */}
          <div className='hidden lg:block lg:col-span-5 bg-surface-container-lowest border border-outline-variant rounded p-6 font-code-sm text-code-sm overflow-x-auto custom-scrollbar shadow-xl relative self-start sticky top-24'>
