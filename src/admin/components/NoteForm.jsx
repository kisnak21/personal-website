...
import { NoteImageUploader } from './NoteImageUploader.jsx'
...
export const NoteForm = ({ note = null, onSuccess, onCancel }) => {
  const queryClient = useQueryClient()
  const isEditing = !!note
  const { data: projects = [] } = useProjects({ all: true })
  ...
  const [activeTab, setActiveTab] = useState('split') // 'edit', 'preview', or 'split'
  ...
  const mutation = useMutation({
    mutationFn: (data) => {
      ...
      return isEditing ? updateNote(note.id, payload) : createNote(payload)
    },
    onSuccess: (result) => {
      if (result.error) {
        setError(result.error.message || 'An error occurred saving the note')
      } else {
    ...
    <form onSubmit={handleSubmit} className='bg-surface-container p-6 rounded-lg border border-outline space-y-6'>
      <div className='flex justify-between items-center border-b border-outline-variant pb-4'>
        ...
        >
          ...
          {showCheatSheet ? 'Tutup Cheat Sheet Markdown' : 'Markdown Cheat Sheet'}
        </button>
      </div>
      ...
      {showCheatSheet && (
        <div className='bg-surface-container-highest p-4 rounded border border-secondary/40 text-xs space-y-3'>
          <div className='font-bold text-secondary flex items-center justify-between'>
            ...
            <span className='font-normal text-on-surface-variant'>Bisa dikopi langsung atau diketik di area content</span>
          </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
        ...
        <div>
          ...
          <input
            ...
            required
            placeholder='e.g., Membangun Arsitektur Buildfolio'
        ...
        <div>
          ...
          <input
            ...
            required
            placeholder='membangun-arsitektur-buildfolio'
        ...
        <div>
          <label className='block font-label-caps text-xs font-semibold text-on-surface mb-1'>
            Terkait dengan Project (Opsional)
          </label>
          <select
            ...
            className='w-full bg-surface border border-outline-variant rounded px-3 py-2 text-sm text-on-surface focus:outline-none focus:border-primary'
          >
            <option value=''>-- Tanpa Project Terkait --</option>
        ...
        <div>
          <label className='block font-label-caps text-xs font-semibold text-on-surface mb-1'>
            Sort Order (Angka kecil tampil duluan)
          </label>
          <input
            type='number'
      ...
      <div>
        <label className='block font-label-caps text-xs font-semibold text-on-surface mb-1'>
          Excerpt / Ringkasan Singkat (Untuk Card Preview)
        </label>
        <textarea
          rows='2'
          value={formData.excerpt}
          onChange={(e) => setFormData(prev => ({ ...prev, excerpt: e.target.value }))}
          placeholder='Ringkasan 1-2 kalimat untuk ditampilkan pada daftar card notes...'
          className='w-full bg-surface border border-outline-variant rounded px-3 py-2 text-sm text-on-surface focus:outline-none focus:border-primary'
        />
      </div>
      ...
      <div>
        ...
        <div className='flex gap-2 mb-2'>
          <input
            ...
            placeholder='e.g., React, Architecture, Learning'
          ...
          <button
            ...
            className='bg-surface-variant hover:bg-surface-container-highest text-on-surface px-4 py-2 rounded text-sm transition-colors font-medium'
          >
            Add
          </button>
        </div>
          ...
          {formData.tags.map(tag => (
            <span key={tag} className='bg-primary/10 text-primary border border-primary/20 font-label-caps text-xs px-2.5 py-1 rounded flex items-center gap-1.5'>
              ...
              <button
                ...
                className='hover:text-error transition-colors focus:outline-none flex items-center'
              >
      ...
      {/* Content Section with Split/Tab View */}
      <div className='border border-outline-variant rounded overflow-hidden'>
        <div className='bg-surface-container-high px-4 py-2 border-b border-outline-variant flex items-center justify-between'>
          <label className='font-label-caps text-xs font-semibold text-on-surface'>
            Markdown Content *
          </label>
          <div className='flex gap-1 bg-surface rounded p-0.5 border border-outline-variant'>
            <button
              ...
              className={`px-3 py-1 rounded text-xs font-code-sm ${activeTab === 'edit' ? 'bg-primary text-on-primary' : 'text-on-surface-variant hover:text-on-surface'}`}
            >
              Edit Only
            </button>
            <button
              ...
              className={`hidden md:block px-3 py-1 rounded text-xs font-code-sm ${activeTab === 'split' ? 'bg-primary text-on-primary' : 'text-on-surface-variant hover:text-on-surface'}`}
            >
              Split View
            </button>
            <button
              ...
              className={`px-3 py-1 rounded text-xs font-code-sm ${activeTab === 'preview' ? 'bg-primary text-on-primary' : 'text-on-surface-variant hover:text-on-surface'}`}
            >
              Preview Only
            </button>
          </div>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 min-h-[450px]'>
          ...
          <div className={`${activeTab === 'preview' ? 'hidden' : 'block'} ${activeTab === 'edit' ? 'md:col-span-2' : ''} border-r border-outline-variant flex flex-col`}>
            <textarea
              ...
              required
              placeholder='# Ketik judul atau konten Markdown di sini...'
          ...
          <div className={`${activeTab === 'edit' ? 'hidden' : 'block'} ${activeTab === 'preview' ? 'md:col-span-2' : ''} bg-surface-container-lowest p-6 overflow-y-auto max-h-[550px] custom-scrollbar`}>
            <div className='text-xs font-code-sm text-tertiary mb-3 uppercase tracking-wider pb-2 border-b border-outline-variant'>
            ...
            <div className='markdown-content text-sm'>
              {formData.content ? (
                ...
                <span className='text-on-surface-variant/40 italic'>Preview akan tampil saat kamu mulai mengetik...</span>
              )}
            </div>
          </div>
        </div>
      ...
      <div className='flex items-center justify-between pt-4 border-t border-outline-variant'>
        <label className='flex items-center space-x-2 cursor-pointer select-none'>
          <input
            ...
            className='rounded bg-surface border-outline-variant text-primary focus:ring-primary h-4 w-4'
          />
          <span className='font-label-caps text-sm text-on-surface'>Published (Tampil untuk publik)</span>
        </label>
        ...
        <div className='flex gap-3'>
          {onCancel && (
            <button
              ...
              className='px-4 py-2 rounded border border-outline text-on-surface-variant hover:bg-surface-variant transition-colors text-sm font-medium'
            >
              Cancel
            </button>
          )}
          <button
            type='submit'
            disabled={mutation.isPending}
            className='bg-primary hover:bg-primary-container text-on-primary font-medium px-6 py-2 rounded text-sm transition-colors flex items-center space-x-2 disabled:opacity-50'
          >
            {mutation.isPending && (
              <span className='material-symbols-outlined animate-spin text-[16px]'>sync</span>
            )}
            <span>{mutation.isPending ? 'Saving...' : isEditing ? 'Update Note' : 'Create Note'}</span>
          </button>
        </div>
      </div>
    </form>
