...
export function SkillForm({ skill = null, onSuccess, onCancel }: SkillFormProps) {
  ...
  const mutation = useMutation({
    mutationFn: async (data: typeof formData) => {
      const url = isEditing ? `/api/admin/skills/${skill.id}` : '/api/admin/skills'
  ...
  const colorOptions = [
    ...
    { value: 'secondary', label: 'Secondary', bg: 'bg-secondary' },
    { value: 'tertiary', label: 'Tertiary', bg: 'bg-tertiary' },
  ]

  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
      {error && (
        <div className="p-4 bg-error/10 border border-error/30 text-error rounded font-code-sm text-sm">
          {error}
        </div>
      )}
      ...
      <div className="space-y-4">
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-2 bg-surface border border-outline-variant rounded text-on-surface focus:border-primary focus:outline-none"
